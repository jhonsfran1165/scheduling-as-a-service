...
init-$(1): dependencies network-create prebuild-$(1)
endef
...
build-$(1): prebuild-$(1)
	@[ -f "docker-compose.override.yaml" ] || cp -n docker-compose.yaml docker-compose.override.yaml;
...
start-$(1):
	@[ -f "docker-compose.override.yaml" ] || cp -n docker-compose.yaml docker-compose.override.yaml;
	@docker-compose pull "$(1)"; docker-compose -f docker-compose.override.yaml up -d "$(1)"
endef
...
stop-$(1):
	@[ -f "docker-compose.override.yaml" ] || cp -n docker-compose.yaml docker-compose.override.yaml;
	@docker-compose -f docker-compose.override.yaml stop "$(1)"
endef
...
###############################################################################
settings_dir := $(shell find . -type d -name "oceana_*")

.PHONY: clean
clean:
	@[ -f "docker-compose.override.yaml" ] || cp -n docker-compose.yaml docker-compose.override.yaml;
	@[ -f ".env" ] || cp -n .env.example .env;
	@docker-compose -f docker-compose.override.yaml down -v --rmi local --remove-orphans || true
	@rm -rf .env docker-compose.override.yaml
